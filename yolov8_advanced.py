...
experiment_config = {
    ...
    "hsv_s": 0.7,
    "hsv_v": 0.4,
    "degrees": 5.0,  # Rotational augmentation for cars
    "translate": 0.2,
    "scale": 0.5,
    "shear": 0.0,
    "perspective": 0.0,
    ...
    "mosaic": 1.0,
    "mixup": 0.2,
    # "cls_wts": [1.0, 3.0, 3.0, 3.0, 4.0, 3.0, 2.0, 2.0],
    "copy_paste": 0.0,
...
def calculate_class_weights(class_counts):
    ...
    normalized = {cls: min(weight/max_weight * 3, 4.0) for cls, weight in inverse_freqs.items()}
    return normalized

# After dataset organization but before training
class_counts = {"Car": 28742, "Cyclist": 1627, "Misc": 973, "Pedestrian": 4487, 
                "Person_sitting": 222, "Tram": 511, "Truck": 1094, "Van": 2914}
...
def reorganize_kitti_dataset_optimized():
    ...
    data_yaml = {
        ...
        "class_weights": [1.0, 3.0, 3.0, 3.0, 4.0, 3.0, 2.0, 2.0]       
    }
...
def train_yolov8_model():
    ...
    # Add this check to prevent loading checkpoints from different model sizes
    if checkpoint_path and f"yolov8{experiment_config['model_size']}" not in checkpoint_path:
        print(f"Found checkpoint {checkpoint_path} but it's for a different model size. Starting fresh.")
